...
**The browser reports the deadline it measures, rather than acting on it alone.** Keeping the session alive while the user is inside their inactivity window means the last *request* runs ahead of the last *interaction*, and each renewal extends by a full window from whenever that request landed — so the session outlives the deadline it exists to enforce, by however long the user had already been quiet. A browser that logs itself out at its own deadline therefore leaves a live session behind it, and one that is closed at the wrong moment leaves it with nobody left to end it. So the keep-alive call carries the deadline as a cap: `expires_at` then tracks the user's last interaction, the session ends when the inactivity window says it should whether or not that browser is still running, and the client has no reason to log itself out. It cannot be abused, because the cap can only ask for *less* than the configured window and cannot shorten an expiry already granted — the worst a client can do with it is decline to extend its own session.
