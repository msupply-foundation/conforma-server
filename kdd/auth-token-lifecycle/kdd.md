...
- **`externalApiConfigs` needs two new auth types.** A peer Conforma is the §4 mechanism reversed, sending a provisioned long-lived credential as a cookie, so there is no login step on that route. mSupply was assumed to be Basic auth and is not — its v4 API hands out an expiring session only through a login call — so a second type logs in for it and logs in again when the session is rejected: the existing cookie jar is the cache, one de-duplicated login per API, one replay (§7, revised).
...
**Snapshots dump and restore it like every other table, and carry the restoring admin's own session across on top.** A snapshot is a faithful copy of the database, and `user_session` is part of the database; singling it out for exclusion would make a snapshot something subtly less than a copy of what it was taken from. It also has a concrete cost: a machine client's provisioned session (§4) is a credential someone has already handed out and cannot re-derive, and an exclusion would destroy it on every snapshot round-trip while looking like housekeeping. The accepted consequence is that restoring another system's snapshot brings that system's sessions with it. They are only usable by whoever holds the matching refresh token, and a restore is admin-triggered — an actor who already has `role: 'postgres'` — so this grants nobody anything they could not take anyway.

What a restore *does* need is a way for the admin to survive it. [`useSnapshot.ts:96`](../../src/components/snapshots/useSnapshot.ts#L96) deliberately runs `DROP SCHEMA public CASCADE` before `pg_restore`, so nothing in `public` survives to be replaced by the incoming rows — the caller's own session included. No dump or restore flag can help with that, since the row that must survive does not exist in either database. Instead, at the start of `useSnapshot` the **restoring admin's own session row is read out, together with their username**, and written back afterwards ([`sessionRestore.ts`](../../src/components/permissions/sessionRestore.ts)). Nothing else is preserved, and nothing incoming is removed.

**It is re-resolved by username, not re-inserted verbatim.** A restore replaces the whole dataset, so the stored `user_id` may now identify a different person — the hazard `routeUserInfo` already guards against by re-checking the JWT's username against a fresh lookup. So the row goes back only if that username exists in the restored data, rewritten to whatever `user_id` it now has. If the username is absent, nothing is re-inserted and the admin logs in against the restored system, which is the honest outcome — they have no account on it. That case is already recoverable on non-live servers through the existing `USER_PASSWORD_OVERRIDE` reset ([`migrateData.ts:1535`](../../database/migration/migrateData.ts#L1535)).

`org_id` gets the same treatment for the same reason, but only the system org (id 1) can survive it: that one is baked into every snapshot and means the same organisation in any dataset, whereas any other id addresses a row that has just been replaced. So it is kept when it is the system org and cleared otherwise, leaving the user to pick an org again.

The write is an upsert, because the restored dump may already contain this very `token_hash` — a snapshot taken from this same server carries the session that is now asking to be reinstated, pointing at whichever user held that id when the snapshot was taken. And the reinstated expiry is floored at one inactivity window from now, by the same `GREATEST` clamp renewal uses: a restore can take longer than the window, and putting back an already-expired row would be indistinguishable from not putting it back at all.

Two things fall out of this that are easy to miss. The **sweep has to be suspended** for the duration of a restore ([`sessionCleanup.ts`](../../src/components/permissions/sessionCleanup.ts)): it reports a session as ended when a connected client's row is absent, and for the whole restore the table is either gone or the incoming snapshot's, so it would tell every client — the restoring admin first — that their session had ended, undoing the preservation from the other side. And the **access cookie has to be discarded** on the way out ([`routeUseSnapshot.ts`](../../src/components/snapshots/routes/routeUseSnapshot.ts)): it stays signature-valid for up to an hour, so renewal would not fire, and its `userId`, `orgId` and `pp<policyId>` claims all describe the database that has just been replaced. Expiring it leaves the refresh cookie alone, so the next request takes the ordinary renewal path and rebuilds every claim from the preserved row.
...
**"Active" must mean user activity, not requests.** The server only sees requests, so a user typing into a long form for twenty minutes would be logged out while actively working — a regression, because today's `LoginInactivityTimer` watches mouse and keyboard. **The existing idle tracker is reused rather than replaced**; it simply changes job. Instead of logging the user out when the deadline passes, it calls the server while the user is interacting, which keeps the session alive and — should the access cookie have expired — has it silently replaced by the same middleware as any other request.

For that call to keep the session alive it must extend `expires_at` **whether or not a token was minted**, where everything else extends it only as a side effect of minting. `POST /api/heartbeat` is that call: a single extend-and-report, deliberately not `/api/user-info`, which rebuilds the user's whole picture on every call and is far more than is needed to say someone is still working.
...
**The browser reports activity, and the session is extended from the request like any other.** Keeping the session alive while the user is inside their inactivity window means the last *request* runs ahead of the last *interaction*, and each renewal extends by a full window from whenever that request landed — so the session outlives the deadline it exists to enforce, by however long the user had already been quiet. A browser that logs itself out at its own deadline therefore leaves a live session behind it, and one that is closed at the wrong moment leaves it with nobody left to end it.

The gap is closed by *how often* activity is reported rather than by what the report carries: the error is then bounded by the reporting interval, and it always falls the safe way — a session ends slightly early, never late — so the orphan case cannot arise however the browser goes away. The cost is a few requests per window instead of one, which is what `POST /api/heartbeat` above is sized for.

> **Superseded.** This originally had the keep-alive carry the browser's measured deadline as a *cap*, so `expires_at` tracked the last interaction exactly (`GREATEST(expires_at, LEAST(now + window, cap))`). It was built and then removed: exactness was not worth a query parameter, a `LEAST` in the renewal write, and a deadline that had to be shared across tabs so a background tab could not cut the active one's session short. Frequency buys a bound that is good enough, with none of that machinery.
...
> **Superseded** on the mSupply side ([conforma-server#1169](https://github.com/msupply-foundation/conforma-server/issues/1169)). The premise that neither direction acquires an expiring credential was wrong: mSupply was assumed to be Basic auth and is not. Its v4 API offers neither a long-lived token nor any auto-renew mechanism — only a login call that sets a session cookie and expires it — so the only way to hold a working credential is to log in ourselves and log in again when the session lapses. `CookieLogin` builds the minimum of what this paragraph declined. The cookie jar `CookieToken` already keeps is the cache, so there is no second store; a login promise held on the session is the de-duplication — its presence is the "logging in" flag and awaiting it is the queue — so twenty concurrent rejections produce one login; and a rejected request is replayed once, decided by a per-session generation counter that only a successful login bumps, so a rejection that lands after someone else's login has already completed retries without logging in again. A failed login backs off (`loginFailTimeout`) but keeps the cookies it holds, since the failure may say nothing about them. It is smaller than the `LoginEndpoint` sketch below because mSupply delivers the session as `Set-Cookie`: there is no token path and no cookie name to configure, and the login body is static strings rather than a FigTree expression, since the session is per API and shared by every caller — a body carrying per-user data would serve one user's session to another. The prefs-reload problem is avoided the same way the jar already avoids it: the session is keyed by a hash of the credential, so editing it orphans the old session by construction.
...
- **`externalApiConfigs` stays stateless.** Because both routes use a static credential, the relay remains a pure per-request function of config: a credential edit takes effect immediately, with no cache to invalidate on prefs reload and no acquisition step that could fail or leak a login response into logs.
  > **Superseded** for `CookieLogin` (§7): the relay holds a per-API session — cookies, an in-flight login and a failure backoff — in memory, so it is no longer a pure per-request function of config. What the consequence was protecting survives: a credential edit still takes effect immediately, because the session is keyed by a hash of the credential and an edit orphans it by construction rather than through a reload hook; and the acquisition step logs its attempt and outcome status only, never the login body, the response, a cookie value or the fingerprint.
