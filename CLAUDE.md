...
- **The archive store is immutable.** Archived files (`file.archive_path` set) are never deleted or moved, even when their DB record goes; the file cleanup keeps records of archived files it cannot find and warns instead. Which archives a system needs is derived from the `file` table, never from the store's `archive.json`. See [documentation/Backups.md](documentation/Backups.md#file-archives).
