...
- Sessions are also how Conforma authenticates **to another Conforma**: `externalApiConfigs`' `ConformaSession` auth type ([../external-apis/authHeaders.ts](../external-apis/authHeaders.ts)) sends a peer-provisioned token as the refresh cookie, and lets the peer's renewal path do the rest. Same mechanism, pointed outward.
