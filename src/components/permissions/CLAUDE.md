...
- **JWT shape** (`compileJWT` in [rowLevelPolicyHelpers.ts](rowLevelPolicyHelpers.ts)):
  ...
  ```
- Verified with `config.jwtSecret` (env `JWT_SECRET`; resolved by `resolveJwtSecret()` in [../../config.ts](../../config.ts), which exits a production build when it is missing or blank and otherwise falls back to the public `'devsecret'` with a startup warning). Both surfaces read that one value — a second reading of the environment would risk the two disagreeing and rejecting every token the other issued. The REST `preValidation` hook in [../../server.ts](../../server.ts) populates `request.auth`; PostGraphile independently verifies the same token for GraphQL and exposes claims as `current_setting('jwt.claims.*')`.
...
- Sessions are also how Conforma authenticates **to another Conforma**: `externalApiConfigs`' `CookieToken` auth type ([../external-apis/authHeaders.ts](../external-apis/authHeaders.ts)) sends a peer-provisioned token as the refresh cookie, then carries the access token the peer mints back to it ([cookieJar.ts](../external-apis/cookieJar.ts)) so the peer only mints again once that one expires. Same mechanism, pointed outward.
