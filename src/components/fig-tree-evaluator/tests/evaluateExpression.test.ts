/**
 * Test suite for FigTree Evaluator module. FigTree itself has comprehensive
 * tests, but the purpose of this is to test backwards compatibility. These
 * tests were written for v1 of the evaluator, so we want to ensure the tests
 * still pass so we can be confident that existing template configurations will
 * work with the  new Evaluator without any modifications.
 *
 * For the most part this should be possible, but there is a couple of things to
 * watch out for:
 *
 * - we normally want `supportDeprecatedValueNodes` set to `true` to handle the
 *   old-style "value" nodes, but this can cause problems with some of the HTTP
 *   operators if they happen to have a parameter named `value`
 * - `evaluateFullObject` would normally be desirable, but can cause problems
 *   with the aforementioned "value" nodes
 *
 */
...
test('GET: Check username is unique', () => {
  return figTree
    .evaluate(testData.APIisUnique, {
      ...
      // Required here, otherwise it breaks up parameters.value
...
test('GET: Lookup ToDo in online testing API', () => {
  return figTree
    .evaluate(testData.onlineTestAPI, {
      // APIfetch: fetch,
    })
    .then((result: any) => {
...
test('GET: Return an array from online API', () => {
  return figTree
    .evaluate(testData.onlineArrayReturn, {
      // APIfetch: fetch,
    })
    .then((result: any) => {
...
test('GET: Return an array of titles plucked from inside array of objects', () => {
  return figTree
    .evaluate(testData.onlineArrayNodes, {
      // APIfetch: fetch,
    })
    .then((result: any) => {
...
test('Test Postgres lookup single string', () => {
  return figTree.evaluate(testData.getApplicationName).then((result: any) => {
...
test('Test Postgres get array of template names', () => {
  return figTree.evaluate(testData.getListOfTemplates).then((result: any) => {
    expect(result).toEqual([
      'User Registration',
      'Edit User Details',
...
test('Test Postgres get Count of templates', () => {
  return figTree.evaluate(testData.countTemplates).then((result: any) => {
    expect(result).toEqual(24)
  })
})

test('Test Postgres get template names -- no type', () => {
  return figTree.evaluate(testData.getListOfTemplates_noType).then((result: any) => {
    expect(result).toEqual([
...
test('Test Postgres get application list with IDs', () => {
  return figTree.evaluate(testData.getListOfApplications_withId).then((result: any) => {
    expect(result).toEqual([
...
test('Test GraphQL -- get continents list from External Graphql API ', () => {
  return figTree
    .evaluate(testData.GraphQL_GetContinentsList_ExternalAPI, {
      // graphQLConnection: {
      //   fetch: fetch,
      //   endpoint: graphQLendpoint,
      // },
...
test('Test GraphQL -- get country details by code from External Graphql API ', () => {
  return figTree
    .evaluate(testData.GraphQL_GetCountryByCode_ExternalAPI, {
      // graphQLConnection: {
      //   fetch: fetch,
      //   endpoint: graphQLendpoint,
      // },
...
test('Test GraphQL -- Check result of field can be null', () => {
  return figTree
    .evaluate(testData.GraphQL_CheckDefaultResultIsNull, {
      // graphQLConnection: {
      //   fetch: fetch,
      //   endpoint: graphQLendpoint,
      // },
...
test('Compare null vs undefined', () => {
  return figTree.evaluate({ operator: '=', children: [null, undefined] }).then((result: any) => {
    expect(result).toEqual(true)
  })
})
