...
export const installTemplate = async (
  ...
  try {
    ...
    // Deliberately not awaited, so the import response isn't held up by the
    // checksum retry loop -- see updateLinkedEntityData below
    if (sourceFolder) updateLinkedEntityData(newTemplateId, template.code)
...
// linked_entity_data can't be generated within the import transaction, as the
// linked entities' checksums are computed asynchronously (via DB
// notifications) after their records are written. So we retry until they've
// settled. If it still fails, the value gets regenerated by the "self-heal"
// mechanism next time the template is checked, duplicated or exported (see
// ensureLinkedEntityData).
const LINKED_ENTITY_UPDATE_MAX_ATTEMPTS = 10
const LINKED_ENTITY_UPDATE_RETRY_DELAY = 2000 // ms

const updateLinkedEntityData = async (templateId: number, code: string) => {
  for (let attempt = 1; attempt <= LINKED_ENTITY_UPDATE_MAX_ATTEMPTS; attempt++) {
    try {
      const linked_entity_data = await getTemplateLinkedEntities(templateId)
      await db.updateRecord('template', { linked_entity_data, id: templateId })
      console.log(`Updated linked_entity_data for imported template: ${code}`)
      return
    } catch (err) {
      // A missing referenced file is a data problem -- no point retrying
      if ((err as ApiError).message.includes('missing from the database')) {
        console.log(
          `WARNING: unable to generate linked_entity_data for template ${code} - ${
            (err as ApiError).message
          }`
        )
        return
      }
      await new Promise((resolve) => setTimeout(resolve, LINKED_ENTITY_UPDATE_RETRY_DELAY))
    }
  }
  console.log(
    `WARNING: linked_entity_data could not be updated for imported template ${code}. It will be regenerated next time the template is checked, duplicated or exported.`
  )
}
