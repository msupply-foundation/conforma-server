...
export const PREFERENCES_FILE = path.join(PREFERENCES_FOLDER, config.preferencesFileName)
export const GENERIC_THUMBNAILS_FOLDER = path.join(
  getAppEntryPointDir(),
  config.imagesFolder,
  config.genericThumbnailsFolderName
)
export const BASE_SNAPSHOT_FOLDER = path.join(
  getAppEntryPointDir(),
  DATABASE_FOLDER,
  BASE_SNAPSHOT_NAME
)
