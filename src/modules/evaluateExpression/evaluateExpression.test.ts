...
test("Test returning single user property", () => {
  return evaluateExpression(testData.singleUserProperty, { user: testData.user }).then((result: any) => {
...
test("Test Postgres lookup single string", () => {
  return evaluateExpression(testData.getApplicationName, { connection: pgConnect }).then((result: any) => {
    expect(result).toBe("Company registration: Company C");
  });
});

test("Test Postgres get array of template names", () => {
  return evaluateExpression(testData.getListOfTemplates, { connection: pgConnect }).then((result: any) => {
    expect(result).toEqual(["User Registration", "Company Registration"]);
  });
});

test("Test Postgres get Count of templates", () => {
  return evaluateExpression(testData.countTemplates, { connection: pgConnect }).then((result: any) => {
    expect(result).toEqual(2);
  });
});

test("Test Postgres get template names -- no type", () => {
  return evaluateExpression(testData.getListOfTemplates_noType, { connection: pgConnect }).then((result: any) => {
    expect(result).toEqual([{ template_name: "User Registration" }, { template_name: "Company Registration" }]);
  });
});
...
test("Test concatenate user First and Last names", () => {
  return evaluateExpression(testData.concatFirstAndLastNames, { user: testData.user }).then((result: any) => {
    expect(result).toBe("Carl Smith");
  });
});
...
test("Test Trigger condition -- Stage = 1 (Screening) and All Questions are approved", () => {
  return evaluateExpression(testData.complex2, { application: testData.application, connection: {} }).then(
    (result: any) => {
      expect(result).toBe(true);
    }
  );
...
test("Test Trigger condition -- Stage = 1 (Screening) and All Questions are approved -- input as Stringified JSON", () => {
  return evaluateExpression(testData.complex2_asString, { application: testData.application, connection: {} }).then(
    (result: any) => {
      expect(result).toBe(true);
    }
  );
